...
class RamTab(BaseTab):
    ...
    Fully expanded to track individual physical layout per memory slot dynamically.
    Protected with active main loop scrollregion calculations to prevent layout freezes.
    """
    def __init__(self, parent, font_registry):
        ...
        group_ram = self.create_group_box(" TELEMETRIA DE VOLUMES E ALOCAÇÃO DA RAM ", target_parent=self.scroll_content_frame)
        ...
        self.group_hw = self.create_group_box(" MAPEAMENTO DE SLOTS DA PLACA-MÃE (DMI/SMBIOS) ", border_color="#555555", target_parent=self.scroll_content_frame)
        self.append_grid_row(self.group_hw, "Slots de Memória Ocupados no Hardware:", "ram_hw_slots_summary", text_color="#ffffff")

        # --- BOX 3: Advanced Linux Kernel Memory Profiles Box Group ---
        group_advanced = self.create_group_box(" CACHE DE KERNEL E ÁREA DE TROCA (SWAP) ", border_color="#9cdcfe", target_parent=self.scroll_content_frame)
        ...
        for dev in hw_report.get("devices", []):
            row_f = tk.Frame(self.group_hw, bg="#2d2d2d")
            row_f.pack(fill="x", expand=True, padx=15, pady=3)
            if hasattr(self, 'row_frames'): self.row_frames.append(row_f)
            
            lbl_title = tk.Label(row_f, text=f" ❑  Encaixe [{dev['slot']}]:", font=self.fonts["label"], bg="#2d2d2d", fg="#aaaaaa")
            lbl_title.pack(side="left", anchor="w")
            if hasattr(self, 'name_labels'): self.name_labels.append(lbl_title)
            
            if dev["size"] == "Vazio":
                val_text = "Slot Vazio (Disponível para Upgrade)"
                val_fg = colors["text_sec"] if 'colors' in locals() else "#aaaaaa"
            else:
                val_text = f"{dev['size']} | {dev['type']} | {dev['speed']} | {dev['vendor']} ({dev['form']})"
                val_fg = "#ffffff"
                
            lbl_value = tk.Label(row_f, text=val_text, font=self.fonts["monospace"], bg="#2d2d2d", fg=val_fg)
